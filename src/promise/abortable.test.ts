...
describe(AbortablePromise, () => {
  ...
  it("is chainable", async () => {
    const abThatResolves = new AbortablePromise((resolve) => resolve("resolved"));

    await expect(abThatResolves.then((value) => "then " + value)).resolves.toEqual("then resolved");
...
describe(AbortablePromise.abort, () => {
  it("creates an AbortablePromise that has been aborted", async () => {
    await expect(AbortablePromise.abort()).rejects.toEqual(AbortablePromise.AbortError);
  });
});
...
describe(AbortablePromise.resolve, () => {
  it("creates an AbortablePromise that will resolve to the given value unless aborted", async () => {
    await expect(AbortablePromise.resolve("resolved")).resolves.toEqual("resolved");
...
describe(AbortablePromise.reject, () => {
  it("creates an AbortablePromise that will reject with the given reason unless aborted", async () => {
    await expect(AbortablePromise.reject("rejected")).rejects.toEqual("rejected");
...
describe("AbortablePromise.then", () => {
  it("returns an AbortablePromise", async () => {
    expect(new AbortablePromise(() => {}).then()).toBeInstanceOf(AbortablePromise);
  });
...
describe(`${abortable.name}`, () => {
  ...
  it("preserves a Call that already returns an AbortablePromise", async () => {
    ...
    await expect(abortable(call)()).resolves.toEqual("resolved AbortablePromise");

    const rejectingCall = () => AbortablePromise.reject("rejected AbortablePromise");
    await expect(abortable(rejectingCall)()).rejects.toEqual("rejected AbortablePromise");

    const abortingCall = () => AbortablePromise.abort();
    await expect(abortable(abortingCall)()).rejects.toEqual(AbortablePromise.AbortError);
  });
});
